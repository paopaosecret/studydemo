...
public class DragListView extends ListView {
	...
	public boolean onTouchEvent(MotionEvent ev) {
		...
		switch (action) {
			case MotionEvent.ACTION_DOWN:
				...
				if (mDragPosition == AdapterView.INVALID_POSITION || mDownX > getWidth() * 1 / 4) {
	...
	public void setmPositionChangeListener(PositionChangeListener mPositionChangeListener) {
		...
		if (mFromPosition != mToPosition) {
			if (exchangePosition()) {
				...
				if(mPositionChangeListener != null){
					mPositionChangeListener.onPositionChange(mFromPosition,mToPosition);
				}
